...
const useTouchEvent = (el: Element, event: EventInterface) => {
  const onTouchStart = () => {
    ...
    timeoutId.value = window.setTimeout(async () => {
      ...
      if (
        Math.abs(touchStartclientY.value - touchEndclientY.value) <=
          antiTouchDistanse &&
        !isScrolling.value
      ) {
        longLock.value = true;
...
const initLongTouch = (el: Element, binding: DirectiveBinding): void => {
...
const uninstallLongTouch = (el: Element, binding: DirectiveBinding): void => {
  const { onTouchStart, onTouchMove, onTouchEnd } = useTouchEvent(
    el,
    binding.instance as EventInterface
  );
